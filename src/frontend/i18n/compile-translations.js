 ...
 *
 * Utility copied from project Richie https://github.com/openfun/richie.git
 * formatjs/cli compile and compile-folder methods do not allow merge.
 * So we need our own compile method.
 *
...
// Transparently includes Ashley default translation files to lighten command
const LOCALES_PATTERN = path.resolve(__dirname, `./locales/*.json`);
// Output directory
const OUT_DIR =
  yargs.argv.outDir || path.resolve(__dirname, `../js/translations`);
...
(async () => {
  ...
  const files = await Promise.all(
    ...
    matches.flat().reduce((outputs, filePath) => {
