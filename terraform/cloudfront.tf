...
locals {
  static_origin_id = "ashley-static-origin"
  s3_media_origin_id = "ashley-media-origin"
}
...
resource "aws_cloudfront_origin_access_identity" "ashley_oai" {
  comment = "ashley origin for the ${terraform.workspace} environment"
}

resource "aws_cloudfront_distribution" "ashley_cloudfront_distribution" {
  ...
  origin {
    ...
    s3_origin_config {
      origin_access_identity = aws_cloudfront_origin_access_identity.ashley_oai.cloudfront_access_identity_path
    }
  }
